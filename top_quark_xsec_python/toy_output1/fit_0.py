...
first_guess.append([19000.9, 660.0, 360.0, 3040.0, 140.0]) # njets = 4, for toy studies
...
params_dict['xsec_ttbar'] = {'fix':False,'start_val':227,'limits':(100,1000),'error':1}
...
for i,s in enumerate(samples[0:-1]):
    for j in range(njets_min,njets_max+1):
        ...
        name = "num_%s_njets%d" % (s,j)
        if s=='ttbar':
            params_dict[name] = {'fix':True,'start_val':first_guess[j][i],'limits':(0,nd),'error':1}
        else:
            params_dict[name] = {'fix':False,'start_val':first_guess[j][i],'limits':(0,nd),'error':1}

        #'''
        name = "nominal_%s_njets%d" % (s,j)
        if s=='ttbar':
            params_dict[name] = {'fix':True,'start_val':nominal[j][i],'limits':(0,nd),'error':1}
        else:
            params_dict[name] = {'fix':True,'start_val':nominal[j][i],'limits':(0,nd),'error':1}

        name = "uncert_%s_njets%d" % (s,j)
        if s=='ttbar':
            params_dict[name] = {'fix':True,'start_val':uncert[j][i],'limits':(0,nd),'error':1}
        else:
            params_dict[name] = {'fix':True,'start_val':uncert[j][i],'limits':(0,nd),'error':1}
        #'''
